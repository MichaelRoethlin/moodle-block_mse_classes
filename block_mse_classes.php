    ...
    /**
     * mse_classes block caps.
     *
     * @package    block_mse_classes
    ...
    class block_mse_classes extends block_list
    {
        ...
        public function get_content()
        {
            ...
            $url1 = new moodle_url('/blocks/mse_classes/bookings.php', array('courseid' => $courseid));
            $this->content->items[] = html_writer::tag('a', 'Bookings for ' . $modulecode, array('href' => 'some_file.php'));

            $url2 = new moodle_url('/blocks/mse_classes/confirmation.php', array('id' => $courseid));
            $this->content->items[] = html_writer::link($url2, 'Presence Confirmation');

            $url3 = new moodle_url('/blocks/mse_classes/inscription.php', array('userid' => $USER->id, 'courseid' => $courseid));
            $this->content->items[] = html_writer::link($url3, 'Inscriptions');

            if (has_capability('block/mse_classes:manage', $coursecontext)) {
                $url4 = new moodle_url('/blocks/mse_classes/information.php', array('userid' => $USER->id, 'id' => $courseid));
